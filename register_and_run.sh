...
if [ -z ${GITLAB_INSTANCE_URL+x} ]; then
    ...
    export CI_SERVER_URL=http://$(mesosdns-resolver --serviceName $GITLAB_SERVICE_NAME --server $MESOS_DNS_SERVER --portIndex 0 --drill)
else
    # Display the GitLab instance URL discovery method
...
dind dockerd --host=unix:///var/run/docker.sock --storage-driver=overlay2 $DOCKER_EXTRA_OPTS &
